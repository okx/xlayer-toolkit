...
#
# Usage:
#   ./list-game.sh [--l1-rpc <url>] [--dgf <address>] [game_type] [start_index end_index]
#
# Examples:
#   ./list-game.sh                                    # List all games (uses .env config)
...
#   ./list-game.sh --l1-rpc http://rpc:8545 --dgf 0xABC...  # Override RPC and factory
#   ./list-game.sh --dgf 0xABC... 42                 # Override factory, list type 42
...
# Fetch claimData(), createdAt(), resolvedAt(), status(), bondDistributionMode() in one JSON-RPC batch (requires jq).
# Falls back to sequential cast calls if jq is unavailable.
# Returns: "status_text|deadline_ts|created_ts|resolved_ts|close_text"
get_game_data() {
    local addr=$1
    local claim_hex created_hex resolved_hex game_status_hex bond_mode_hex

    if command -v jq &> /dev/null; then
        # ── JSON-RPC batch: one HTTP round-trip for all 5 calls ──────
        ...
        local sig_bond=$(cast sig "bondDistributionMode()")
        local body
        printf -v body \
            '[{"jsonrpc":"2.0","id":1,"method":"eth_call","params":[{"to":"%s","data":"%s"},"latest"]},{"jsonrpc":"2.0","id":2,"method":"eth_call","params":[{"to":"%s","data":"%s"},"latest"]},{"jsonrpc":"2.0","id":3,"method":"eth_call","params":[{"to":"%s","data":"%s"},"latest"]},{"jsonrpc":"2.0","id":4,"method":"eth_call","params":[{"to":"%s","data":"%s"},"latest"]},{"jsonrpc":"2.0","id":5,"method":"eth_call","params":[{"to":"%s","data":"%s"},"latest"]}]' \
            "$addr" "$sig_claim" "$addr" "$sig_created" "$addr" "$sig_resolved" "$addr" "$sig_status" "$addr" "$sig_bond"
        local resp
        ...
        bond_mode_hex=$(echo "$resp" | jq -r '.[] | select(.id==5) | .result // ""')
    else
        ...
        bond_mode_hex=$(cast call "$addr" "bondDistributionMode()" --rpc-url "$L1_RPC" 2>/dev/null)
    fi
    ...
    local bond_mode=$(cast --to-dec "$bond_mode_hex" 2>/dev/null || echo "0")
    ...
    case $bond_mode in
        ...
        1) close_text="NORMAL" ;;
        2) close_text="REFUND" ;;
        *) close_text="Unknown($bond_mode)" ;;
    esac

    echo "$status_text|$deadline|$created_ts|$resolved_ts|$close_text"
}
...
list_games() {
    ...
    # Table header
    printf "${BOLD}%-6s %-8s %-12s %-22s %-7s %-14s %-13s %-13s %-13s %-10s${NC}\n" \
        "Index" "Type" "Parent" "Block Range" "Blocks" "Status" "CreatedAt" "Deadline" "ResolvedAt" "Close"
    echo "────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────"
    ...
    for ((i=total-1; i>=0; i--)); do
        ...
        if [ "$parent" = "$GENESIS_PARENT_INDEX" ]; then
            parent_display="anchor"
            start=$(cast call $addr "startingBlockNumber()(uint256)" --rpc-url $L1_RPC 2>/dev/null | awk '{print $1}')
            [ -z "$start" ] && start="?"
            if [ "$start" != "?" ]; then
        ...
        local status_text deadline_ts created_ts resolved_ts close_text
        IFS='|' read -r status_text deadline_ts created_ts resolved_ts close_text <<< "$game_data"
        ...
        printf "%-6s %-8s %-12s %-22s %-7s %-14s %-13s %-13s %-13s %-10s\n" \
            "$i" "$type" "$parent_display" "$start-$l2_block" "$blocks" \
            "$status_text" "$created_fmt" "$deadline_fmt" "$resolved_fmt" "$close_text"
        count=$((count + 1))
    done
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --dgf)
            ...
            ;;
        --help|-h)
            echo "Usage: $0 [--l1-rpc <url>] [--dgf <address>] [game_type] [start_index end_index]"
            echo ""
            echo "Options:"
            echo "  --l1-rpc <url>      Override L1 RPC URL (default: \$L1_RPC_URL or http://localhost:8545)"
            echo "  --dgf <address>     Override Dispute Game Factory address (default: \$DISPUTE_GAME_FACTORY_ADDRESS)"
            echo ""
            echo "Examples:"
            echo "  $0                                         # List all games"
            echo "  $0 42                                      # List all games of type 42"
            echo "  $0 42 10 20                                # List type 42 games from index 10 to 20"
            echo "  $0 --l1-rpc http://rpc:8545 --dgf 0xABC   # Override RPC and factory"
            echo "  $0 --dgf 0xABC 42                         # Override factory, list type 42"
            exit 0
            ;;
        -*)
        ...
        *)
            if [ -z "$GAME_TYPE" ]; then
                ...
                GAME_TYPE="$1"
            elif [ -z "$FILTER_START" ]; then
                if ! [[ "$1" =~ ^[0-9]+$ ]]; then
                    echo -e "${RED}Error: Start index must be a number${NC}"
                    exit 1
                fi
                FILTER_START="$1"
            elif [ -z "$FILTER_END" ]; then
                if ! [[ "$1" =~ ^[0-9]+$ ]]; then
                    echo -e "${RED}Error: End index must be a number${NC}"
                    exit 1
                fi
                FILTER_END="$1"
            else
...
# Apply CLI overrides (take precedence over .env)
[ -n "$ARG_L1_RPC" ] && L1_RPC="$ARG_L1_RPC"
[ -n "$ARG_DGF" ] && FACTORY_ADDRESS="$ARG_DGF"

if [ -n "$FILTER_START" ] && [ -z "$FILTER_END" ]; then
    echo -e "${RED}Error: start_index requires end_index${NC}"
    exit 1
fi
if [ -n "$FILTER_START" ] && [ "$FILTER_START" -gt "$FILTER_END" ]; then
