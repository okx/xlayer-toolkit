...
if [ -f "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" ]; then
    ...
    FUTEX_ENTER=$(grep -c "syscalls:sys_enter_futex" "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" 2>/dev/null || echo "0")
    FUTEX_EXIT=$(grep -c "syscalls:sys_exit_futex" "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" 2>/dev/null || echo "0")
    FUTEX_WAIT_ENTER=$(grep -c "syscalls:sys_enter_futex_wait" "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" 2>/dev/null || echo "0")
    FUTEX_WAIT_EXIT=$(grep -c "syscalls:sys_exit_futex_wait" "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" 2>/dev/null || echo "0")
    ...
    echo ""
    echo "I/O Operations:"
    READ_COUNT=$(grep -c "syscalls:sys_enter_read" "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" || echo "0")
    WRITE_COUNT=$(grep -c "syscalls:sys_enter_write" "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" || echo "0")
    FSYNC_COUNT=$(grep -c "syscalls:sys_enter_fsync" "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" || echo "0")
    FDATASYNC_COUNT=$(grep -c "syscalls:sys_enter_fdatasync" "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" || echo "0")
    ...
    grep -c "sched:sched_switch" "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" || echo "0"
    echo ""
    echo "Block I/O operations:"
    BLOCK_ISSUE=$(grep -c "block:block_rq_issue" "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" || echo "0")
    BLOCK_COMPLETE=$(grep -c "block:block_rq_complete" "$OUTPUT_DIR/perf-offcpu-${TIMESTAMP}.script" || echo "0")
