...
if [ "$FLASHBLOCK_ENABLED" = "true" ]; then
    ...
    sed_inplace "s/^SEQ_L2_EXECUTION_CLIENT=.*/SEQ_L2_EXECUTION_CLIENT=rollup-boost/" .env
    sed_inplace "s/^SEQ_L2_EXECUTION_PORT=.*/SEQ_L2_EXECUTION_PORT=8552/" .env
    echo "✅ Set sequencer L2 engine to rollup-boost service in .env"
