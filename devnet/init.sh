...
if [ "$SKIP_OP_SUCCINCT_BUILD" = "true" ]; then
    ...
    if [ "$OP_SUCCINCT_DIRECTORY" = "" ]; then
        ...
        build_and_tag_image "op-succinct-contracts" "$OP_SUCCINCT_CONTRACTS_IAMGE_TAG" "$OP_SUCCINCT_DIRECTORY" "$PWD_DIR/op-succinct/Dockerfile.contract"
        build_and_tag_image "op-succinct-proposer" "$OP_SUCCINCT_PROPOSER_IMAGE_TAG" "$OP_SUCCINCT_DIRECTORY" "fault-proof/Dockerfile.proposer"
        build_and_tag_image "op-succinct-challenger" "$OP_SUCCINCT_CHALLENGER_IMAGE_TAG" "$OP_SUCCINCT_DIRECTORY" "fault-proof/Dockerfile.challenger"
    fi
fi