...
| Step 1（读历史）         | Bitable 失败 → 本地文件 → `sent_history = []`（不做去重） | 可能重复推送，可接受 |
...
**Lark Bitable（推荐，跨会话/跨机器）**：若 `pipeline_config.json` 配置了 `paths.sent_history_bitable_url`，使用 Bitable 存储去重历史。详见 Step 1 和 Step 8 的 Bitable 读写逻辑。

**本地文件降级**：Bitable 不可用时，按以下顺序尝试本地文件：
1. `$HOME/intel-data/sent_history_{team_id}.md`
2. `/tmp/sent_history_{team_id}.md`

**⚠️ 关键**：绝对不要写入 `/mnt/.remote-plugins/`（只读）或 `/sessions/xxx/`（临时）。
...
### Step 1：加载去重历史（Lark Bitable 优先）

**优先使用 Lark Bitable**（跨会话、跨机器持久化）：

若 `pipeline_config.json` 中配置了 `paths.sent_history_bitable_url`：

1. **解析 URL**：提取 `wiki_token`（或 `app_token`）和 `table_id`，与 Step 0-C 相同的逻辑
   - `/wiki/` URL → 调用 `lark_wiki_get_node` 获取 `app_token`
   - `/base/` URL → 直接提取 `app_token`

2. **读取历史记录**：调用 `lark_bitable_search_records`，按 team 过滤：
   ```
   searchRecords(
     appToken  = {app_token},
     tableId   = {table_id},
     filter    = { conjunction: "and", conditions: [
       { field_name: "team", operator: "is", value: ["{team_id}"] }
     ]},
     field_names = ["Title", "Summary", "Source_URL", "sent_at"],
     sort      = [{ field_name: "sent_at", desc: true }],
     page_size = 200
   )
   ```

3. **构建去重集合**：从返回的记录中提取 Title 和 Source_URL，用于 Step 4 的去重比对。

**降级方案**：若 Bitable URL 未配置或读取失败 → 回退到本地文件 `sent_history_{team_id}.md`（路径按前置配置中的推导规则）。

**失败处理**：Bitable 读取失败时 `sent_history = []`（不做去重），写日志警告，不中断流水线。
...
### Step 2.7：RSS / 技术博客抓取（双路径：CLI 优先 → 浏览器兜底）

**默认执行**。所有已注册团队均执行 RSS 采集，不限制业务线。

**采集路径选择（按优先级）**：

**路径 A（CLI / 无浏览器环境，优先）**：用 Bash 执行 `fetch_rss.py` 脚本，通过 Python `requests` + XML 解析抓取 RSS，无需浏览器。

先用 Glob 定位脚本：`**/scripts/fetch_rss.py`，然后执行：

```bash
python3 "{FETCH_RSS_SCRIPT}" \
  --max-age-days 3 --max-per-feed 3 \
  "https://blog.ethereum.org/feed.xml" \
  "https://vitalik.eth.limo/feed.xml" \
  "https://ethresear.ch/latest.rss" \
  "https://l2beat.com/blog/feed.xml" \
  "https://rekt.news/rss/" \
  "https://weekinethereumnews.com/feed/"
```

输出为 JSON，包含 `feeds[].items[]`（每条含 title/link/pubDate/summary），将结果追加到内存信号列表，标注 `[source_type: rss]`。

可根据 Step 2 获取的 RSS URL 列表动态构建完整命令（传入所有 P0 + P1 RSS URL）。

**路径 B（浏览器可用时的兜底）**：若路径 A 脚本不存在或执行失败，降级到浏览器 fetch() 方式（原方案）。
...
### Step 8：更新去重历史（Lark Bitable 优先）

将本次推送的所有条目写入去重历史。

**优先使用 Lark Bitable**（与 Step 1 对应）：

若 Step 1 成功使用了 Bitable，则此步也写入 Bitable。对每条推送的 Briefing 条目，调用 `lark_bitable_create_record`：

```
createRecord(
  appToken = {app_token},   // 与 Step 1 相同
  tableId  = {table_id},    // 与 Step 1 相同
  record   = {
    fields: {
      "Title":      "{item.title}",
      "Summary":    "{item.body 前 200 字}",
      "Source_URL":  { "link": "{item.sources[0].url}", "text": "{item.sources[0].name}" },
      "sent_at":    {unix_timestamp_ms},   // 当前时间，Date 类型传 Unix 毫秒时间戳
      "team":       "{team_id}"
    }
  }
)
...
**⚠️ sent_at 字段是 Date 类型**：Lark Bitable Date 字段接收 Unix 毫秒时间戳（整数），不是日期字符串。用 Bash 获取：
```bash
python3 -c "import time; print(int(time.time() * 1000))"
```

**⚠️ Source_URL 是超链接类型**：传入格式为 `{"link": "https://...", "text": "显示名"}`。

**批量写入**：若本次推送多条（≤8 条），可用 `lark_bitable_batch_create_records` 一次写入所有记录，减少 API 调用。

**降级方案**：若 Bitable 写入失败 → 回退到本地文件追加：
```bash
mkdir -p "$HOME/intel-data"
cat >> "$HOME/intel-data/sent_history_{team_id}.md" << 'EOF'
[YYYY-MM-DD HH:MM] | 标题 | 一句话摘要
EOF
```

**失败处理**：写入失败时记录日志但不中断流水线。下次运行可能重复推送，可接受。
