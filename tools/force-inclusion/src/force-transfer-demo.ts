...
// Portal error selectors (cast sig "ErrorName()")
const INSUFFICIENT_DEPOSIT = "0xacb9c385"; // OptimismPortal_InsufficientDeposit
const NOT_ALLOWED_CGT = "0xbd58e0a2";      // OptimismPortal_NotAllowedOnCGTMode
...
const TEST_CASES: TestCase[] = [
  {
    label: "_value=1 OKB, msg.value=0 (CGT compliant, but fails InsufficientDeposit check)",
    msgValue: 0n,
    expectedSelector: INSUFFICIENT_DEPOSIT,
    expectedName: "OptimismPortal_InsufficientDeposit (msg.value != _value)",
  },
  {
    label: "_value=1 OKB, msg.value=1 ETH (violates CGT mode)",
    msgValue: AMOUNT,
    expectedSelector: NOT_ALLOWED_CGT,
    expectedName: "OptimismPortal_NotAllowedOnCGTMode (msg.value > 0)",
  },
];
...
async function main() {
  ...
  console.log(`L2 balance:  ${formatEther(l2Balance)} OKB`);
  console.log(`Amount:      ${formatEther(AMOUNT)} OKB\n`);

  for (const [i, tc] of TEST_CASES.entries()) {
    console.log(`[Attempt ${i + 1}] ${tc.label}`);
    try {
      ...
      if (msg.includes(tc.expectedSelector)) {
        console.log(`  Reverted: ${tc.expectedName}`);
      } else {
        console.log(`  Reverted: ${msg.slice(0, 200)}`);
      }
    }
    if (i < TEST_CASES.length - 1) console.log();
  }
