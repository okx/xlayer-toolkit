...
// 1. Load user's .env (if exists) — these take priority
config();

// 2. Load devnet .env as fallback — won't override existing values
config({ path: DEVNET_ENV_PATH, override: false });

// 3. Map devnet key names to our key names (fallback only)
