#!/bin/bash
# Matrix benchmark — run all combinations of N values, input sizes, and inline option.
#
# Jolt SHA-2 has no proof mode distinction and no GPU support.
# Parameters: N (iterations), INPUT_SIZE (bytes), inline optimization.
#
# Usage:
...
#   ./matrix.sh --input-sizes "32 256"    # custom input sizes in bytes
...
if [ -n "$OUTPUT_CSV" ]; then
    echo "n,input_size,inline,prove_time_s,verify_time_s,peak_memory_mb,peak_cpu_pct,proof_size_bytes" > "$OUTPUT_CSV"
    echo "Results will be saved to: $OUTPUT_CSV"
fi

TOTAL=$(( ${#N_VALUES[@]} * ${#INPUT_SIZES[@]} * ${#INLINE_OPTIONS[@]} ))
CURRENT=0
FAILED=0
...
for n in "${N_VALUES[@]}"; do
    for input_size in "${INPUT_SIZES[@]}"; do
        for inline in "${INLINE_OPTIONS[@]}"; do
            ...
            LABEL="[$CURRENT/$TOTAL] N=$n input_size=$input_size${IL_LABEL}"
            ...
            CMD="N=$n INPUT_SIZE=$input_size MODE=prove INLINE=$inline $SCRIPT_DIR/run.sh run"
            ...
            if [ "$ITERATIONS" -gt 1 ]; then
                OUTPUT=$("$BENCH_DIR/bench.sh" \
                    ...
                    --no-purge \
                    -- \
                    N="$n" INPUT_SIZE="$input_size" MODE=prove INLINE="$inline" \
                    "$SCRIPT_DIR/run.sh" run 2>&1)
            else
                OUTPUT=$(N="$n" INPUT_SIZE="$input_size" MODE=prove INLINE="$inline" \
                    "$SCRIPT_DIR/run.sh" run 2>&1)
            fi
            EXIT_CODE=$?
            set -e
            ...
            if [ $EXIT_CODE -ne 0 ]; then
                ...
                FAILED=$((FAILED + 1))
                if [ -n "$OUTPUT_CSV" ]; then
                    echo "$n,$input_size,$inline,FAIL,FAIL,FAIL,FAIL,FAIL" >> "$OUTPUT_CSV"
                fi
                continue
            fi
            ...
            if [ -n "$OUTPUT_CSV" ]; then
                echo "$n,$input_size,$inline,${PROVE_TIME:-},${VERIFY_TIME:-},${PEAK_MEM:-},${PEAK_CPU:-},${PROOF_SIZE:-}" >> "$OUTPUT_CSV"
            fi
        done
    done
done
