...
use crate::Prover;
use crate::witness::{BlockWitness, WitnessGenerator};
...
impl Proposer {
    ...
    async fn submit_proof(&mut self, game_address: String, disputed_block: u64) -> Result<()> {
        ...
        // 2. Build witness for proof generation
        let (witness, _witness_generator) = {
            let state = self.state.read().await;
            let trace_log = self.trace_log.read().await;
            
            // Get previous state/trace hashes from trace log
            let prev_block = if disputed_block > 1 { disputed_block - 1 } else { 0 };
            let prev_entry = trace_log.get_entry_at(prev_block);
            let prev_state_hash = prev_entry.map(|e| e.state_hash).unwrap_or([0u8; 32]);
            let prev_trace_hash = prev_entry.map(|e| e.trace_hash).unwrap_or([0u8; 32]);
            let prev_smt_root = state.smt_root();
            
            // Create block (TODO: fetch full block with transactions from L2)
            // For now, create empty block - real implementation should fetch from L2
            let block = Block::new(disputed_block, [0u8; 32], 0);
            
            // Create witness generator for future use with real transactions
            let witness_generator = WitnessGenerator::new(state.clone());
            
            // Build witness
            // NOTE: In production, use witness_generator.generate_witness() with actual block data
            // For mock mode, we create a minimal witness that the prover can process
            let witness = BlockWitness {
                block: block.clone(),
                initial_states: vec![],  // Empty for mock, populated from actual txs in production
                smt_proofs: vec![],       // Empty for mock, generated from state in production
                prev_smt_root,
                prev_state_hash,
                prev_trace_hash,
            };
            
            (witness, witness_generator)
        };
        
        // 3. Generate ZK proof
        let proof_result = self.prover.prove(&witness)?;
        info!("  Proof generated: {} bytes", proof_result.proof_bytes.len());
        info!("  Public values: {} bytes", proof_result.public_values.len());

        // 4. Use proof's public_values directly (generated by prover/zkVM)
        // The prover generates correct public_values based on the witness
        let public_values = &proof_result.public_values;

        info!("  Submitting to DisputeGame contract...");

        // 5. Call prove(bytes, bytes) on DisputeGame
