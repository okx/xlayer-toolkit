...
/// @title Poseidon2T4
/// @notice Poseidon2 direct permutation for BN254 (t=4, RF=8, RP=56, x^5).
/// @dev rate=3, capacity=1. hash3(a,b,c) = perm(a,b,c,0)[0].
///      Same permutation as T4 sponge, capacity fixed to 0 (no IV).
///      Constants from Noir/Barretenberg (same as T4 sponge).
library Poseidon2T4 {
    ...
    /// @notice Hash 3 field elements via t=4 permutation with capacity=0.
    function hash3(uint256 a0, uint256 a1, uint256 a2) internal pure returns (uint256) {
        ...
        assembly ("memory-safe") {
            ...
            let s1 := mod(a1, P)     // 0/3
            let s2 := mod(a2, P)     // 0/3
            let s3 := 0              // 0/3 (capacity)

            // ── Initial external layer: M4 ──
            {
                let t0 := add(s0, s1)           // 1/3
                let t1 := add(s2, s3)           // 1/3
                let t2 := add(s1, s1)           // 1/3
                t2 := addmod(t2, t1, P)         // 0/3
                let t3 := add(s3, s3)           // 1/3
                t3 := addmod(t3, t0, P)         // 0/3
                let t4 := mulmod(t1, 4, P)      // 0/3
                t4 := add(t4, t3)               // 1/3
                let t5 := mulmod(t0, 4, P)      // 0/3
                t5 := add(t5, t2)               // 1/3
                s0 := addmod(t3, t5, P)         // 0/3
                s1 := t5                        // 1/3
                s2 := addmod(t2, t4, P)         // 0/3
                s3 := t4                        // 1/3
            }
            ...
            for { let r := 0 } lt(r, 4) { r := add(r, 1) } {
                let base := shl(7, r)
                s0 := add(s0, mload(add(rcPtr, base)))
                s1 := add(s1, mload(add(rcPtr, add(base, 0x20))))
                s2 := add(s2, mload(add(rcPtr, add(base, 0x40))))
                s3 := add(s3, mload(add(rcPtr, add(base, 0x60))))
            ...
            for { let r := 0 } lt(r, 56) { r := add(r, 1) } {
                s0 := add(s0, mload(add(partialBase, shl(5, r))))
                let tmp := s0
                s0 := mulmod(tmp, tmp, P)
                s0 := mulmod(s0, s0, P)
                s0 := mulmod(s0, tmp, P)

                let sum := add(add(add(s0, s1), s2), s3)
            ...
            for { let r := 0 } lt(r, 4) { r := add(r, 1) } {
                let base := shl(7, r)
                s0 := add(s0, mload(add(lastBase, base)))
                s1 := add(s1, mload(add(lastBase, add(base, 0x20))))
                s2 := add(s2, mload(add(lastBase, add(base, 0x40))))
                s3 := add(s3, mload(add(lastBase, add(base, 0x60))))
