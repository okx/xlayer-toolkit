...
if check_result_legacy_tolerant "$response" "eth_getInternalTransactions (legacy tx)"; then
    ...
    log_info "  → Found $internal_txs internal transactions"
fi
...
if echo "$response" | jq -e '.error' > /dev/null 2>&1; then
    log_success "All-zero tx hash returns error ✓"
    PASSED_TESTS=$((PASSED_TESTS + 1))
elif echo "$response" | jq -e '.result == null' > /dev/null 2>&1; then
    log_success "All-zero tx hash returns null (correct) ✓"
    PASSED_TESTS=$((PASSED_TESTS + 1))
...
if echo "$response" | jq -e '.result == null' > /dev/null 2>&1; then
    ...
    log_success "Random tx hash returns empty array (acceptable) ✓"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
...
if check_result_legacy_tolerant "$response" "eth_getInternalTransactions (simple transfer)"; then
    ...
    if [ "$result" = "null" ] || [ "$result" = "[]" ]; then
        log_info "  → Correctly returns null/empty for simple transfer"
    else
        internal_count=$(echo "$result" | jq 'length')
        log_info "  → Found $internal_count internal transactions"
    fi
fi

echo ""
log_info "📊 Phase 12.1 Summary:"
echo "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
if check_result_legacy_tolerant "$response" "eth_getBlockInternalTransactions (legacy)"; then
    ...
    if [ "$result" != "null" ]; then
        ...
        log_info "  → Found $tx_count transactions with internal txs"
    fi
fi
...
if check_result_legacy_tolerant "$response" "eth_getBlockInternalTransactions (earliest)"; then
    result=$(echo "$response" | jq -r '.result')
    if [ "$result" = "null" ] || [ "$result" = "[]" ]; then
        log_info "  → Genesis block has no transactions (expected)"
    fi
fi
...
if check_result_legacy_tolerant "$response" "eth_getBlockInternalTransactions (latest)"; then
    ...
    if [ "$result" != "null" ]; then
        tx_count=$(echo "$result" | jq 'length')
        log_info "  → Latest block: found $tx_count transactions with internal txs"
    fi
fi
...
result=$(echo "$response" | jq -r '.result')
if [ "$result" = "null" ] || [ "$result" = "[]" ]; then
    log_success "Genesis block returns null/empty (correct) ✓"
    PASSED_TESTS=$((PASSED_TESTS + 1))
elif echo "$response" | jq -e '.error' > /dev/null 2>&1; then
    log_success "Genesis block returns error (acceptable) ✓"
    PASSED_TESTS=$((PASSED_TESTS + 1))
else
...
log_info "📊 Phase 12 Complete Summary:"
echo "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
log_info "  Tested 2 internal transaction query methods:"
log_info "  ✓ eth_getInternalTransactions (7 tests)"
log_info "  ✓ eth_getBlockInternalTransactions (14 tests)"
