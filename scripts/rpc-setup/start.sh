...
echo "🚀 Starting X Layer Self-hosted RPC node for $NETWORK_TYPE network with $RPC_TYPE..."
...
if [ "$NETWORK_TYPE" = "testnet" ]; then
    ...
    OP_RETH_IMAGE_TAG="xlayer/op-reth:0.0.1"
...
echo "🔍 Checking configuration files for $NETWORK_TYPE with $RPC_TYPE..."
config_files=("$CONFIG_DIR/$ROLLUP_CONFIG" "$CONFIG_DIR/$EXEC_CONFIG" "$CONFIG_DIR/$GENESIS_FILE")
for file in "${config_files[@]}"; do
    if [ ! -f "$file" ]; then
        echo "❌ Error: Configuration file $file does not exist"
        echo "Please run ./init.sh $NETWORK_TYPE $RPC_TYPE first to initialize the node"
        exit 1
    fi
done
...
echo "🐳 Generating docker-compose file for $NETWORK_TYPE with $RPC_TYPE..."
...
EOF
    ...
    # Generate docker-compose.yml for Geth (original)
    cat > docker-compose.yml << EOF
networks:
  xlayer-network:
    name: xlayer-network
...
# Start services
echo "🐳 Starting Docker services for $NETWORK_TYPE with $RPC_TYPE..."
docker compose up -d
