...
echo "🐳 Generating docker-compose file for $NETWORK_TYPE with RPC type: $RPC_TYPE..."
...
# Start services with special handling for op-reth
...
if [ "$L2_ENGINEKIND" = "reth" ]; then
    ...
    echo "⏳ Starting $RPC_TYPE service first and waiting $OP_RETH_WAIT_TIME seconds for it to load the genesis..."
    docker compose up -d $RPC_TYPE
...
echo "  docker logs -f xlayer-${NETWORK_TYPE}-op-geth"
echo ""
echo "🌐 Exposed Ports:"
echo "| Service | Port | Protocol | Purpose |"
echo "|---------|------|----------|---------|"
echo "| op-geth RPC | 8545 | HTTP | JSON-RPC API |"
echo "| op-geth WebSocket | 7546 | WebSocket | WebSocket API |"
