...
var (
	...
	BlockCount = &cli.IntFlag{
		Name:     "block-count",
		Usage:    "How many blocks to replay (not required if continuous mode is enabled)",
		Required: false,
		EnvVars:  opservice.PrefixEnvVar(EnvVarPrefix, "BLOCK_COUNT"),
	}
	Continuous = &cli.BoolFlag{
		Name:    "continuous",
		Usage:   "Continuously fetch and replay blocks from source node",
		EnvVars: opservice.PrefixEnvVar(EnvVarPrefix, "CONTINUOUS"),
	}
