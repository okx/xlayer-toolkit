...
func LoadReplayorConfig(cliCtx *cli.Context, l log.Logger) (ReplayorConfig, error) {
	...
	continuous := cliCtx.Bool(Continuous.Name)
	blockCount := cliCtx.Int(BlockCount.Name)

	// Validate block count configuration
	if !continuous && blockCount == 0 {
		return ReplayorConfig{}, fmt.Errorf("must provide block-count when continuous mode is not enabled")
	}
