...
func (r *Service) Start(ctx context.Context) error {
	...
	//hash = common.HexToHash("0x20ef81af7b86d57d2f479c985b82c99c18576cd8ef62fc2b011c8e4ae34d3a3b")
	...
	r.log.Info("Starting benchmark", "start_block", currentBlock.NumberU64(), "continuous", r.cfg.Continuous)
	...
	if r.cfg.Continuous {
		blockCount = 0 // Not used in continuous mode
	} else {
		blockCount = uint64(r.cfg.BlockCount)
	}

	benchmark := NewBenchmark(r.clients, r.cfg.RollupConfig, r.log, strategy, r.stats, currentBlock, blockCount, r.cfg.Continuous, r.cfg.BenchmarkOpcodes, r.cfg.ComputeStorageDiffs)
	benchmark.Run(cCtx)
